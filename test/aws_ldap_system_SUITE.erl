...
init_per_suite(Config) ->
    ...
    rabbit_ct_helpers:run_setup_steps(Config, [fun aws_ct_ldap_utils:init_slapd/1]).

end_per_suite(Config) ->
    rabbit_ct_helpers:run_teardown_steps(Config, [fun aws_ct_ldap_utils:stop_slapd/1]).

init_per_group(Group, Config) ->
    aws_ct_ldap_utils:init_per_group(Group, Config).

end_per_group(Group, Config) ->
    aws_ct_ldap_utils:end_per_group(Group, Config).
...
validate_ldap_configuration_via_api(Config) ->
    ...
    Certfile = filename:join([CertsDir, "testca", "cert.pem"]),
    Keyfile = filename:join([CertsDir, "testca", "key.pem"]),
    ...
    http_put(
        Config,
        "/aws/ldap/validate/bad-bind-name",
        #{
            'user_dn' => AliceUserDN,
            'password' => Password,
            'servers' => [LocalHost],
    ...
    ?assertEqual(
        <<"invalid LDAP credentials: DN syntax invalid / too long">>,
    ...
    ?assertEqual(
        <<"invalid LDAP credentials: authentication failure">>,
    ...
    ?assertEqual(
        <<"TLS configuration error: cannot use StartTLS on an SSL connection (use_ssl and use_starttls cannot both be true)">>,
