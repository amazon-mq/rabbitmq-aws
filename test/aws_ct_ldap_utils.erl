-module(aws_ct_ldap_utils).

-export([
    vhost_access_query_base/0,
            ...
            {for, [
                {resource, exchange,
                    {for, [
                        ...
                        {permission, read,
                            {match, {string, "${name}"}, {string, "^xch-${username}-.*"}}}
                    ]}},
                {resource, queue,
                    {for, [
                        {permission, configure,
...
init_per_group(Group, Config, ExtraSteps) ->
    ...
    aws_ct_ldap_seed:seed({"localhost", LdapPort}),
...
end_per_group(_Arg, Config, ExtraSteps) ->
    aws_ct_ldap_seed:delete({"localhost", ?config(ldap_port, Config)}),
