...
- **P1, isolated fault (flap-rate).** Every other node is pristine *and* the candidate flaps: its probability crosses the extreme threshold repeatedly within the window. This is the key path for a partial or periodic fault: the failure detector re-normalises a sustained loss downward over time, so a "fraction of time above a threshold" measure starves, but each loss burst still produces a fresh upward crossing. The tell is that the rest of the cluster stays quiet while one node keeps spiking.
- **P2, sustained fault.** The candidate is extreme for most of the window and leads the next node by a wide margin. This covers a severe fault that pins the probability high even when other nodes are mildly elevated by background loss.
...
- The underlying failure detector is a *change* detector: it re-normalises a persistent loss toward a new baseline over time, so the signal is strongest at the onset of a fault and can fade if the fault persists unchanged for minutes. This suits "detect and replace the node early" but means the `window`, `flap_min`, and crossing-threshold defaults are calibrated for onset detection; they may warrant tuning for a given cluster's fault profile.
