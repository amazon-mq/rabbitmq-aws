...
- **P2, sustained fault.** The candidate (highest inbound median) is extreme for most of the window and leads the next node by a wide margin. This is the sustained-hold path for a continuous fault that pins the probability high (verified in vivo: continuous ~50% loss keeps the failure-detector probability pinned near 1.0, it does not re-normalise downward), even when other nodes are mildly elevated by background loss. Its extreme-fraction gate needs most of the window to be extreme, so P2 only engages after the window fills (~`extreme_frac` * `window` ticks); the onset of the same fault is caught earlier by P1, and the two hand off.
...
- **P1, isolated fault (flap-rate).** P1 selects its candidate by *flap count* (not median): the node whose inbound series crosses the extreme threshold the most times. An intermittent, low-duty-cycle fault has a near-zero median (mostly-zero series with brief high spikes), so choosing by median would name a different, quiet node and count no flaps on the actual culprit; choosing by flap count catches this case directly. P1 fires when the flap candidate has at least `flap_min` crossings *and* every other node is pristine on both signals (median below `pristine` and flap count below `flap_min`), so only a truly isolated flapper is attributed. P1 also fires at the *onset* of a continuous loss fault: packet loss is stochastic, so the probability oscillates across the extreme threshold before it pins high, and those crossings trip P1 within a few seconds - well before P2's extreme-fraction gate can fill. In vivo, continuous 50% loss is attributed by P1 at onset and held by P2 once the window fills, the two handing off.
...
- The underlying failure detector reports, for each peer, the fraction of recent inter-arrival gaps that are small relative to the current silence. Under an *intermittent* fault this oscillates (it falls back during the clean intervals), which is why P1 uses flap-rate. Under a *continuous* fault it stays pinned high (verified in vivo at ~50% loss - the probability does *not* re-normalise downward, because most heartbeats still arrive on time over TCP and only a heavy tail stalls), which is why P2 covers the sustained case. The `window`, `flap_min`, and crossing-threshold defaults are validated in vivo: a periodic (intermittent-drop) fault is attributed via P1, and a continuous loss fault is attributed via P1 at onset - stochastic loss flaps the probability before it pins high, so the verdict fires within a few seconds - then held via P2 once the window fills (~`extreme_frac` * `window` ticks, about 24s at the defaults). In each case the verdict holds for the duration of the fault, the healthy nodes are never falsely attributed, and the defaults may warrant tuning for a given cluster's fault profile.
...
- These metrics are exposed on the shared `rabbitmq_prometheus` `/metrics` endpoint, whose other collectors aggregate cluster-wide state (for example quorum-queue/ra status). Under sustained cluster-wide congestion those collectors can block on cross-node queries and make a `/metrics` scrape slow or briefly unavailable, so a scrape gap during congestion is an artifact of the shared endpoint, not a node-health fault, and is not itself a health signal. The node-health collector does not cause or worsen this: it reads the worker with a bounded (~1s) timeout and degrades to emitting nothing if the worker is slow, and the verdict is still computed each tick and can be read directly from the worker. Observed in vivo under uniform latency+jitter on all nodes: the endpoint stalled for tens of seconds and recovered on its own once the congestion cleared, with no node-health errors and no crashes.
